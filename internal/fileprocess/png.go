...
func ParsePNGRegion(path string)[]byte{
	cacheMu.RLock()
	if d,ok:=cache[path];ok{cacheMu.RUnlock();return d}
	cacheMu.RUnlock()

	d,err:=os.ReadFile(path)
	if err!=nil{fmt.Printf("read err:%v\n",err);return nil}
	...
	for i:=33;i<len(d);{
		...
		if string(d[i+4:i+8])=="IDAT"{idat.Write(d[i+8:i+8+l])}
		i+=l+12
	}

	zr,err:=zlib.NewReader(&idat)
	if err!=nil{fmt.Printf("zlib err:%v\n",err);return nil}
	defer zr.Close()

	raw,err:=io.ReadAll(zr)
	if err!=nil{fmt.Printf("read zlib err:%v\n",err);return nil}
	...
	for y,i:=0,0;y<h;y++{
		...
		for x:=0;x<stride;x++{
			var a,b,c byte
			if x>=4{a=pixels[y*stride+x-4]}
			if y>0{b=pixels[(y-1)*stride+x]}
			if x>=4&&y>0{c=pixels[(y-1)*stride+x-4]}
			v:=raw[i];i++
			switch f{
			case 1:v+=a
			case 2:v+=b
			case 3:v+=byte((int(a)+int(b))>>1)
			case 4:
				p:=int(a)+int(b)-int(c)
				pa,pb,pc:=abs(p-int(a)),abs(p-int(b)),abs(p-int(c))
				if pa<=pb&&pa<=pc{v+=a}else if pb<=pc{v+=b}else{v+=c}
			}
			pixels[y*stride+x]=v
		}
	}
